...
int main()
{
    ...
    while(cycle<=500000){
        ...
        if(PC<inPC){
            ...
            cycle++;
            continue;
        }else code.getinstruc( iimage[ (PC - inPC)/4 ] );

        //cout<<code.instruc<<" "<<code.rd<<" "<<code.rs<<" "<<code.rt<<" "<<hex<<code.C<<endl;

        if(code.rd==0) if(!( code.instruc==sll && code.rt==0 && code.C==0 )) error.write0(cycle);
        ...
        if(code.instruc == add){
            ...
            if(!error.Exit()) regis[code.rt] = dimage[ addr/4 ];
            ...
            else if(addr%4==0) regis[code.rt] = sign_extend(dimage[addr/4] >> 24 ,8);
            ...
            unsigned int addr = getmemaddr(regis[code.rs], code.C);
            if(addr>1023 || addr<0) error.addrover(cycle);
            else if(addr%4==0) regis[code.rt] = dimage[addr/4] >> 24;
            ...
            if(addr>1020 || addr<0) error.addrover(cycle);
            if(addr%4!=0) error.misalign(cycle);
            if(!error.Exit()) dimage[addr/4] = regis[code.rt];
